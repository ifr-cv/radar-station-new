...
from RM_serial_py.ser_api import  build_send_packet, receive_packet, Radar_decision, \
    build_data_decision, build_data_radar_all,build_data_radar
...
def hik_camera_get():
    ...
    # 设置设备的一些参数
    set_Value(cam, param_type="float_value", node_name="ExposureTime", node_value=10000)  # 曝光时间
    set_Value(cam, param_type="float_value", node_name="Gain", node_value=10)  # 增益值
    # 开启设备取流
...
# 大恒相机图像获取线程
from daheng_camera import start_grab_and_get_data_size, set_Value, get_Value
from gxipy.gxiapi import *
from gxipy.gxidef import *

def daheng_camera_get(arg1=None, arg2=None):
    global camera_image
    ...
    print(f"找到 {dev_num} 个大恒相机设备！")

    # 自动选择第一个设备
    selected_index = 0
    if selected_index >= dev_num:
        print("设备选择索引错误！")
        return

    cam = device_manager.open_device_by_index(selected_index + 1)

    # 设置参数
    #cam.ExposureAuto.set("Off")
    # set_Value(cam, param_type="float_value", node_name="ExposureTime", node_value=16000)
    # set_Value(cam, param_type="float_value", node_name="Gain", node_value=15.0)
    # set_Value(cam, param_type="float_value", node_name="AcquisitionFrameRate", node_value=100.0)
    
    #     # 设置感兴趣区域（ROI）以获得更大的视野
    # # 获取相机的最大分辨率
    # max_width = cam.SensorWidth.get()
    # max_height = cam.SensorHeight.get()

    # # 设置 ROI 为相机的最大分辨率，以获得最大的视野
    # cam.OffsetX.set(0)  # 设置 ROI 的起始 X 坐标
    # cam.OffsetY.set(0)  # 设置 ROI 的起始 Y 坐标
    # cam.Width.set(max_width)  # 设置 ROI 的宽度为相机的最大宽度
    # cam.Height.set(max_height)  # 设置 ROI 的高度为相机的最大高度
...
elif camera_mode == 'daheng':
    try:
        # 可以传递参数给线程函数
        thread_camera = threading.Thread(target=daheng_camera_get, args=(None, None), daemon=True)
        thread_camera.start()

    except ImportError as e:
        print(f"导入大恒相机相关模块失败: {e}")
    except Exception as e:
        print(f"创建大恒相机线程时出现错误: {e}")
